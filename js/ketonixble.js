...
var apiKey = require('./js/secrets').apiKey;
...
var incr = 0;
var lastValue = 0;
var lastSubmit = new moment(new Date()).subtract(15, 'seconds');
...
function submitToThingSpeak(value) {
    const a = moment(lastSubmit);
	...
	if (    duration.asSeconds() < 15) {
        $("#status-message").text("Rate limited...");
        return;
    }
	let data = {
		api_key: apiKey,
	...
	incr++;
    $("#status-message").text("Submitting (" + incr + ").");
    fetch("https://api.thingspeak.com/update.json",
		...
		.then((data) => {
		    ...
		    lastSubmit = new Date();
		    if (data == 0 ) {
                $("#status-message").text("Update failed to submit ThingSpeak.");
                incr--;

            }
		    if (data.field1 == value) {
                $("#status-message").text(value + " submitted to ThingSpeak.");
                lastValue = value
                incr--;
            }
...
function createRadialGauge(theunit, green, yellow, red) {
    ...
    if (currentUnit == "PPM") {
        if (zoomOn == true) {
            ...
            if (green < 10 && yellow > 10) {
                theHighlights[1] = new highlight(green, 10, 'rgba(0,255,0,.70)');
            }
            if (green < 10 && yellow < 10) {
            ...
            if (yellow < 10 && red > 10) {
                theHighlights[2] = new highlight(yellow, 10, 'rgba(255,255,0,.80)');
            }
            if (red < 10) {
                theHighlights[3] = new highlight(red, 10, 'rgba(255,0,0,.70)');
            }
        } else {
...
var app = {
    ...
    initialize: function () {
        ...
        $(window).on("orientationchange", function (event) {
            ...
            submitToThingSpeak(gaugePPM.value);
            gaugePPM.draw();
            ;
	...
	*/
        ...
        $(".delete-data-point").on("click", function () {
            alert("delete-data-point");
        });
    ...
    writeCalibration: function (val) { // CALIBRATION
        ...
        device.read(function updateIt(err, data) {
            ...
            if (old_deviceType > 10) {
                deviceType = 2;
            } else {
                deviceType = old_deviceType;
            }
            ...
            data2[0] = 0x0;
            data2[1] = 0x36;
            data[16] = (deviceType >> 0);
            data[17] = (deviceType >> 8);
            device.write(data2);
